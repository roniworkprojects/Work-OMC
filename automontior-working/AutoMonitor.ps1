...
copy C:\testAlexeym\Ronii\InstallNagiosOnLinux.txt.bak C:\testAlexeym\Ronii\InstallNagiosOnLinux.txt -ErrorAction Ignore
copy C:\testAlexeym\Ronii\InstallNagios-MonitorcPanel.txt.bak C:\testAlexeym\Ronii\InstallNagios-MonitorcPanel.txt -ErrorAction Ignore
copy C:\testAlexeym\Ronii\NagiosOnMonitor.txt.bak C:\testAlexeym\Ronii\NagiosOnMonitor.txt -ErrorAction Ignore
...
do {
	...
	while (!$pwMon) {write-host "Please enter ROOT password for " -foreground "green" -nonewline
		write-host "WEBSITE NAME - *IPADRESS*" -foreground "Yellow" -backgroundcolor "Black" -nonewline
...
#				}
	...
	if ($cPanel -eq "1") 
		{
		$fileNameMon = " C:\testAlexeym\Ronii\InstallNagios-MonitorcPanel.txt"
		copy $fileNameMon $fileNameMon".bak"
		$fileNameMonBak = " C:\testAlexeym\Ronii\InstallNagios-MonitorcPanel.txt.bak"
		do {
		...
		$fileNameMon = "C:\testAlexeym\Ronii\NagiosOnMonitor.txt"
		copy $fileNameMon $fileNameMon".bak"
		$fileNameMonBak = "C:\testAlexeym\Ronii\NagiosOnMonitor.txt.bak"
		do {
			write-host "`nIf you need, enter another custom Nagios Service, one at a time, otherwise, leave empty."
			write-host "To add service enter: check_zyx or nsca_check_xyz (e.g. check_ftp)."
			write-host "To remove service enter prefix '-!' (e.g. -!check_https)." 
	...
	$fileName = "C:\testAlexeym\Ronii\InstallNagiosOnLinux.txt"
	copy $fileName $fileName".bak"
	$fileNameBak = "C:\testAlexeym\Ronii\InstallNagiosOnLinux.txt.bak"
